...
class EasyFloat {
    ...
    class Builder(private val activity: Context) : OnPermissionResult {
        ...
        /**
         * 设置浮窗的布局视图，以及布局的操作接口
         * @param layoutViewBuilder 自定义的布局视图Builder
         * @param invokeView 布局视图的操作接口
         */
        @JvmOverloads
        fun setLayout(layoutViewBuilder:(View)-> View, invokeView: OnInvokeView? = null) = apply {
            config.layoutViewBuilder = layoutViewBuilder
            config.invokeView = invokeView
        }
        ...
        fun show() = when {
            // 未设置浮窗布局文件/布局视图，不予创建
            config.layoutId == null && config.layoutView == null&&config.layoutViewBuilder==null ->
